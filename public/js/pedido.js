...
(async () => {
  try {
    const redirigiendo = await window.panelAuth.requireAuth();
    if (redirigiendo) return; // requireAuth ya está navegando a Auth0, no seguir
    ...
    document.getElementById("contenido").innerHTML =
      `<div class="estado-error">Error al iniciar la página: ${err.message}</div>`;
    console.error(err);
  }
})();
